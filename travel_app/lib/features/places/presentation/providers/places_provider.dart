...
Future<CachedLocalTime> currentLocalTime(CurrentLocalTimeRef ref, LatLng coordinates) async {
   // --- Caching Logic ---
   final cacheLink = ref.keepAlive();
   Timer? timer;
   ref.onDispose(() => timer?.cancel());
   ref.onResume(() => timer = Timer(const Duration(minutes: 15), () {
       print("Local time cache expired for $coordinates. Closing keepAlive link.");
       cacheLink.close();
   }));
   // Set initial timer
   timer = Timer(const Duration(minutes: 15), () {
       print("Local time cache expired for $coordinates (Initial). Closing keepAlive link.");
       cacheLink.close();
   });
   // --- End Caching Logic ---

   print("PROVIDER: Fetching time for ${coordinates.latitude}, ${coordinates.longitude}");
   try {
      // Call your actual API function to get the time string
      final timeString = await getCurrentTimeFromLatLng(coordinates.latitude, coordinates.longitude);

      // Calculate expiry time
      final expiresAt = DateTime.now().add(const Duration(minutes: 15));
      print("PROVIDER: Fetched time for ${coordinates.latitude}, ${coordinates.longitude}. Expires at $expiresAt");


      // Return the data object
      return CachedLocalTime(timeString: timeString, expiresAt: expiresAt);
   } catch (e) {
      print("PROVIDER: Error fetching time for ${coordinates.latitude}, ${coordinates.longitude}: $e");
