...
class CityPlacesGrid extends ConsumerStatefulWidget {
  final AsyncValue<List<PlaceByCity>> placesInCityAsync; // Receive the AsyncValue

  const CityPlacesGrid({
    super.key,
    required this.placesInCityAsync,
  });
...
class _CityPlacesGridState extends ConsumerState<CityPlacesGrid>
    with TickerProviderStateMixin { // Need Ticker for TabController
  ...
  void _processPlacesData(List<PlaceByCity> places) {
    ...
    for (final place in places) { (grouped[place.category] ??= []).add(place); }
    final uniqueCategories = grouped.keys.toList()..sort();
    bool categoriesChanged = !const ListEquality().equals(_categories, uniqueCategories);
    Map<String, int> newCurrentPages = {};

    for (final category in uniqueCategories) {
      newCurrentPages[category] = (categoriesChanged ? 0 : _categoryCurrentPage[category]) ?? 0;
    }

    if (mounted) {
      bool needNewController = categoriesChanged && (_tabController == null || _tabController!.length != uniqueCategories.length);
      TabController? oldController = _tabController;
      TabController? newController; // Declare outside setState

      if (needNewController) {
        if (uniqueCategories.isNotEmpty) {
          newController = TabController(length: uniqueCategories.length, vsync: this);
  ...
  void _handleTabSelection() {
    if (mounted && _tabController != null && !_tabController!.indexIsChanging) {
      setState(() { /* Trigger rebuild to show correct grid for the new tab index */ });
    }
  }
  // --- End Listeners ---
    ...
    return widget.placesInCityAsync.when(
      data: (places) {
        ...
        if (_tabController == null || _categories.isEmpty) {
          ...
          if (places.isNotEmpty && _categories.isEmpty) return const Padding(padding: EdgeInsets.symmetric(vertical: 16.0), child: Text("Processing places..."));
          return const Padding( padding: EdgeInsets.symmetric(vertical: 16.0), child: Text("No specific places found nearby.") );
        }
        ...
        return Column(
          ...
          children: [
            ...
            TabBar(
              ...
              isScrollable: true,
              indicatorWeight: 2.0, indicatorPadding: EdgeInsets.zero,
              labelPadding: const EdgeInsets.symmetric(horizontal: 12.0), padding: EdgeInsets.zero,
              labelColor: Theme.of(context).primaryColor,
              unselectedLabelColor: Colors.grey[600],
              indicatorColor: Theme.of(context).primaryColor,
              tabs: _categories.map((category) {
                 final displayName = category.replaceAll('_', ' ').capitalizeFirst();
                 return Tab(text: displayName);
              }).toList(),
              onTap: (index) {
                 // Optional: Trigger rebuild immediately on tap
                 if (mounted && _tabController?.index == index) { setState(() {}); }
              },
            ),
            ...
            AnimatedSwitcher(
               ...
               child: Container(
                  key: ValueKey<int>(_tabController!.index), // Key based on tab index
                  child: buildPaginatedCategoryGrid(_categories[_tabController!.index]),
               ),
            )
          ],
        );
      ...
      loading: () => const Center(
        child: Padding(
          ...
          child: Column( children: [ CircularProgressIndicator(), SizedBox(height: 8), Text("Loading places...") ] ),
        ),
      ),
      error: (err, stack) => Center(
        child: Padding(
          padding: EdgeInsets.symmetric(vertical: 16.0),
          child: Text("Error loading places: $err", style: TextStyle(color: Colors.red)),
        ),
      ),
    );
  }
  ...
  Widget buildPaginatedCategoryGrid(String category) {
    ...
    if (categoryPlaces.isEmpty) return const Center(child: Text("No places in this category."));
    ...
    final List<PlaceByCity> itemsForCurrentPage = categoryPlaces.sublist(startIndex, endIndex);
    ...
    return Column(
      ...
      children: [
        ...
        GridView.builder(
          ...
          physics: const NeverScrollableScrollPhysics(), // Keep physics override
          ...
          itemBuilder: (context, index) {
            ...
            return Card(
               ...
               child: InkWell(
                  onTap: () { print("Tapped Place: ${place.name}"); /* TODO: Handle tap */ },
                  child: Column(
                     ...
                     children: [
                        Expanded(
                           child: Container(
                              ...
                              child: useDefaultPlaceImage
                                 ? Image.asset('assets/city.png', fit: BoxFit.cover)
                                 : (placeImageUrl != null
                                    ? Image.network(
                                        placeImageUrl,
                                        fit: BoxFit.cover,
                                    ...
                                    : const Icon(Icons.image_not_supported, color: Colors.grey)), // Fallback
                           ),
                        ),
                        Padding(
                           padding: const EdgeInsets.all(6.0),
                           child: Text(place.name, style: Theme.of(context).textTheme.bodySmall, textAlign: TextAlign.center, maxLines: 1, overflow: TextOverflow.ellipsis),
                        ),
                     ],
                  ),
               ),
        ...
        if (totalPages > 1)
          Padding(
            ...
            child: _buildDotIndicator(totalPages, currentPage, (index) {
              ...
              setState(() { _categoryCurrentPage[category] = index; });
            }),
          )
      ],
    );
  ...
  Widget _buildDotIndicator(int pageCount, int currentPage, ValueChanged<int> onDotTapped) {
     const int maxDotsToShow = 7;
     int startDot = 0; int endDot = pageCount;
     if (pageCount > maxDotsToShow) { startDot = max(0, currentPage - (maxDotsToShow ~/ 2)); endDot = startDot + maxDotsToShow; if (endDot > pageCount) { endDot = pageCount; startDot = endDot - maxDotsToShow; }}
        ...
        children: List.generate(endDot - startDot, (indexInRow) {
           ...
           return GestureDetector(
             ...
             child: AnimatedContainer(
               ...
               height: isActive ? 10.0 : 8.0, width: isActive ? 10.0 : 8.0,
               decoration: BoxDecoration( color: isActive ? Theme.of(context).primaryColor : Colors.grey[400], shape: BoxShape.circle),
             ),
           );
        }),
     );
